...
class Register():
	...
	def __new__(self, reg):
		...
		REGISTER_INDEX_MESSAGE = '{YELLOW}Invalid register index: {CYAN}`{__reg__}`{RESET}'
		REGISTER_LIMIT_MESSAGE = '{YELLOW}Invalid register index: {CYAN}`{__reg__}`{YELLOW}, ' \
			'limit is {PURPLE}{__limit__}{RESET}'
		REGISTER_FORMAT_MESSAGE = '{YELLOW}Invalid register format: {CYAN}`{__reg__}`{RESET}'
		...
		if st.startswith(('a', 't', 'x')) and st[1].isdigit():
			try:
				...
				raise RegisterError(REGISTER_INDEX_MESSAGE.replace('{__reg__}', st[1:]))
			match st[0]:
				case 'a':
					if reg_cnt > 7:
...
class Instruction():
	...
	Rtype = {'slli', 'srli', 'srai', 'add', 'sub', 'sll', 'slt', 'sltu', 'xor', 'srl', 'sra', 'or', 'ans'}
	Itype = {'jalr', 'lb', 'lh', 'lw', 'lbu', 'lhu', 'addi', 'slti', 'sltiu', 'xori', 'ori', 'andi', 'fence', 'ecall', \
		'ebreak'}
	Stype = {'sb', 'sh', 'sw'}
	Btype = {'beq', 'bne', 'blt', 'bge', 'bltu', 'bgeu'}
	Utype = {'lui', 'auipc'}
	Jtype = {'jal'}
	Pseudo = {'la', 'nop', 'mv', 'not', 'neg', 'seqz', 'sneq', 'sltz', 'sgez', 'beqz', 'bnez', 'blez', 'bgez', 'bltz', \
	...
	def parse(self, line, instr_cnt):
		...
		while split[0].endswith(':'):
			...
			if not label.isalnum():
				raise RISCvLabelError("label name can contain only digits and letters", 0)
		...
		def Rtype(self, line):
			...
			instr_bin = '{funct7}{rs2}{rs1}{funct3}{rd}{opcode}'.format(
				funct7=funct7, rs2=rs2, rs1=rs1, funct3=funct3, opcode=opcode
			)
			return [instr]
		...
		def Itype(self, line):
			...
			instr_bin = '{imm}{rs1}{funct3}{rd}{opcode}'.format(
				imm=imm, rs1=rs1, funct3=funct3, rd=rd, opcode=opcode
			)
			return [instr]
		...
		def Stype(self, line):
			...
			instr_bin = '{imm_11_5:07b}{rs2:05b}{rs1:05b}{funct3:03b}{imm_4_0:05b}{opcode:07b}'.format(
				imm_11_5=imm[11:5], rs2=rs2, rs1=rs1, funct3=funct3, imm_4_0=imm[4:0], opcode=opcode
			)
